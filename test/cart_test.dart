...
void main() {
  ...
  test('adding zero quantity does not change the cart', () {
    final item = menuItem('zero-quantity-item');

    controller.sync([item]);
    controller.add(item, quantity: 0);
  ...
  test('fresh juice varieties remain separate cart lines after sync', () {
    final juice =
        FoodItem.seed.firstWhere((item) => item.id == 'juice');
